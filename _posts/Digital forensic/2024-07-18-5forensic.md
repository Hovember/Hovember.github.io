...
일반 디렉터리 엔트리에는 파일이름의 6번째까지만 기록되며 ~!로 끝나게 된다. LongLo~! 
추가 엔트리를 할당 받아 긴파일명을 처음부터 표시한다. 
긴 파일명 엔트리 확인하기 위해서는 맨 앞에 시퀀스 번호와 속성이 0x0F인지 확인하면 된다. 
