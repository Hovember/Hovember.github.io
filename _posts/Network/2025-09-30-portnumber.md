...
외부에서 공유기 내 특정 장치에 접속하기 위해서 IP 주소를 알아야하지만 공유기 내부는 사설 IP로 이루어져 있어 외부 IP로만 특정 장치를 지정할 수 없다.
따라서 공유기의 공인 IP로 접속한 후 특정한 장치로 연결되도록 포트 넘버를 공유기에 정해주는 작업이 필요하다.
이러한 과정을 포트 포워딩이라 한다. 
